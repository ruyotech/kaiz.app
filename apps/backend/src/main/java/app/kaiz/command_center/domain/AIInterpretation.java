...
/**
 * AI response that may require clarification before creating a draft. Supports two modes: 1. READY
 * - Draft is complete, can be created immediately 2. NEEDS_CLARIFICATION - Need to ask user
 * questions first
 */
public record AIInterpretation(
  ...
  /** Flow for gathering missing information. Max 3-5 questions to keep it focused. */
  ...
  public static AIInterpretation ready(
    ...
    return new AIInterpretation(
        InterpretationStatus.READY, type, confidence, draft, reasoning, suggestions, null, null);
  }

  /** Create a response that needs clarification. */
  public static AIInterpretation needsClarification(
      DraftType suggestedType, Draft partialDraft, String reasoning, ClarificationFlow flow) {
    return new AIInterpretation(
