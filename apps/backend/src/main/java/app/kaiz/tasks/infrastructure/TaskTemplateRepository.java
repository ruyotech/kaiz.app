...
public interface TaskTemplateRepository extends JpaRepository<TaskTemplate, UUID> {
  ...
  @Query("SELECT t FROM TaskTemplate t WHERE t.creatorType = :creatorType ORDER BY t.usageCount DESC")
  List<TaskTemplate> findGlobalByUsageCount(@Param("creatorType") CreatorType creatorType);

  // Templates by type (task/event)
  List<TaskTemplate> findByCreatorTypeAndTypeOrderByRatingDesc(CreatorType creatorType, TemplateType type);

  // Templates by life wheel area
  @Query("SELECT t FROM TaskTemplate t WHERE t.creatorType = 'SYSTEM' AND t.defaultLifeWheelArea.id = :areaId ORDER BY t.rating DESC")
  List<TaskTemplate> findGlobalByLifeWheelArea(@Param("areaId") String areaId);

  @Query("SELECT t FROM TaskTemplate t WHERE t.user.id = :userId AND t.defaultLifeWheelArea.id = :areaId ORDER BY t.name ASC")
  List<TaskTemplate> findByUserIdAndLifeWheelArea(@Param("userId") UUID userId, @Param("areaId") String areaId);
  ...
  @Query("SELECT t FROM TaskTemplate t WHERE t.creatorType = 'SYSTEM' OR t.user.id = :userId ORDER BY t.creatorType ASC, t.rating DESC")
  ...
  // Top rated templates
  @Query("SELECT t FROM TaskTemplate t WHERE t.creatorType = 'SYSTEM' AND t.ratingCount >= :minRatings ORDER BY t.rating DESC")
