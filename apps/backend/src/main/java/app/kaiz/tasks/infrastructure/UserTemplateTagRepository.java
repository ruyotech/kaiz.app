...
public interface UserTemplateTagRepository extends JpaRepository<UserTemplateTag, UUID> {
  ...
  @Query(
      "SELECT utt.tag FROM UserTemplateTag utt WHERE utt.user.id = :userId AND utt.template.id = :templateId ORDER BY utt.tag")
  List<String> findTagsByUserIdAndTemplateId(
      @Param("userId") UUID userId, @Param("templateId") UUID templateId);
  ...
  Optional<UserTemplateTag> findByUserIdAndTemplateIdAndTag(
      UUID userId, UUID templateId, String tag);
