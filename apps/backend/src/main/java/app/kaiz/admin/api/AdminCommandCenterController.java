...
/**
 * Admin endpoints for managing Command Center configuration.
 * Includes LLM providers, system prompts, test attachments, settings, and feature flags.
 */
@RestController
@RequestMapping("/api/v1/admin/command-center")
@RequiredArgsConstructor
@Tag(name = "Admin Command Center", description = "Admin endpoints for Command Center configuration")
...
public class AdminCommandCenterController {
    ...
    public ResponseEntity<ApiResponse<List<SystemPromptResponse>>> getPromptsByCategory(
        ...
        return ResponseEntity.ok(ApiResponse.success(commandCenterService.getPromptsByCategory(category)));
    }
    ...
    public ResponseEntity<ApiResponse<SystemPromptResponse>> getPromptByKey(@PathVariable String key) {
    ...
    public ResponseEntity<ApiResponse<List<TestAttachmentResponse>>> getTestAttachmentsByType(
        ...
        return ResponseEntity.ok(ApiResponse.success(commandCenterService.getTestAttachmentsByType(type)));
    }

    @GetMapping("/test-attachments/{id}")
    @Operation(summary = "Get test attachment by ID")
    public ResponseEntity<ApiResponse<TestAttachmentResponse>> getTestAttachment(@PathVariable UUID id) {
    ...
    public ResponseEntity<byte[]> downloadTestAttachment(@PathVariable UUID id) {
        ...
        return ResponseEntity.ok()
                ...
                .header("Content-Disposition", "attachment; filename=\"" + attachment.attachmentName() + "\"")
                .body(data);
    }
    ...
    public ResponseEntity<ApiResponse<TestAttachmentResponse>> updateTestAttachment(
            @PathVariable UUID id,
            @Valid @RequestBody UpdateTestAttachmentRequest request) {
        return ResponseEntity.ok(ApiResponse.success(commandCenterService.updateTestAttachment(id, request)));
    }
    ...
    public ResponseEntity<ApiResponse<FeatureFlagResponse>> updateFeatureFlag(
        ...
        return ResponseEntity.ok(ApiResponse.success(commandCenterService.updateFeatureFlag(key, request)));
    }
}
