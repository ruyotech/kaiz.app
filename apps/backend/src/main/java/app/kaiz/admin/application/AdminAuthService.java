...
public class AdminAuthService {
  ...
  public AdminAuthResponse login(
      String email, String password, String deviceInfo, String ipAddress) {
    AdminUser admin =
        adminUserRepository
            .findByEmail(email)
            .orElseThrow(
                () ->
                    new ApiException(
                        HttpStatus.UNAUTHORIZED, "UNAUTHORIZED", "Invalid credentials"));
  ...
  public AdminAuthResponse refreshToken(String refreshToken) {
    ...
    AdminRefreshToken storedToken =
        adminRefreshTokenRepository
            .findByTokenHash(tokenHash)
            .orElseThrow(
                () ->
                    new ApiException(
                        HttpStatus.UNAUTHORIZED, "UNAUTHORIZED", "Invalid refresh token"));

    if (!storedToken.isValid()) {
      throw new ApiException(
          HttpStatus.UNAUTHORIZED, "UNAUTHORIZED", "Refresh token expired or revoked");
    }
    ...
    AdminRefreshToken newTokenEntity =
        AdminRefreshToken.builder()
            .admin(admin)
  ...
  public AdminDto getCurrentAdmin(UUID adminId) {
    AdminUser admin =
        adminUserRepository
            .findById(adminId)
            .orElseThrow(
                () -> new ApiException(HttpStatus.NOT_FOUND, "NOT_FOUND", "Admin not found"));
  ...
  public AdminDto createAdmin(
      String email, String password, String fullName, AdminUser.AdminRole role) {
