...
import app.kaiz.sensai.domain.Intervention;
...
public interface InterventionRepository extends JpaRepository<Intervention, UUID> {
  ...
  @Query(
      "SELECT i FROM Intervention i WHERE i.user.id = :userId "
  ...
  @Query(
      "SELECT COUNT(i) FROM Intervention i WHERE i.user.id = :userId "
          + "AND i.triggeredAt >= :since")
  int countInterventionsSince(@Param("userId") UUID userId, @Param("since") Instant since);
  ...
  @Query(
      "SELECT i.interventionType, COUNT(i) FROM Intervention i "
  ...
  boolean existsByUserIdAndInterventionTypeAndIsActiveTrue(UUID userId, InterventionType type);
}
