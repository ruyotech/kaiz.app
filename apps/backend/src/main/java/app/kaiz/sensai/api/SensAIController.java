...
/**
 * REST API controller for SensAI - the AI Scrum Master. Provides endpoints for standups,
 * interventions, velocity, ceremonies, and settings.
 */
@RestController
...
public class SensAIController {
  ...
  public ResponseEntity<ApiResponse<List<DailyStandupDto>>> getStandupHistory(
    ...
    return ResponseEntity.ok(
        ApiResponse.success(sensAIService.getStandupHistory(userId, startDate, endDate)));
  }
  ...
  public ResponseEntity<ApiResponse<DailyStandupDto.StandupSummary>> getStandupSummary(
      @CurrentUser UUID userId, @RequestParam(defaultValue = "30") int days) {
  ...
  public ResponseEntity<ApiResponse<DailyStandupDto>> skipStandup(
      @CurrentUser UUID userId, @Valid @RequestBody DailyStandupDto.SkipStandupRequest request) {
  ...
  @Operation(
      ...
      description = "Get all active/pending interventions")
  ...
  @Operation(
      ...
      description = "Get paginated intervention history")
  public ResponseEntity<ApiResponse<Page<InterventionDto>>> getInterventionHistory(
    ...
    return ResponseEntity.ok(
        ApiResponse.success(sensAIService.getInterventionHistory(userId, pageable)));
  }

  @PostMapping("/interventions/{interventionId}/acknowledge")
  @Operation(
      summary = "Acknowledge intervention",
      description = "Acknowledge and optionally take action on an intervention")
  ...
  @Operation(
      summary = "Dismiss intervention",
      description = "Dismiss an intervention with a reason")
  public ResponseEntity<ApiResponse<InterventionDto>> dismissIntervention(
    ...
    return ResponseEntity.ok(
        ApiResponse.success(sensAIService.dismissIntervention(userId, interventionId, request)));
  }
  ...
  @Operation(
      summary = "Get sprint health",
      description = "Get health assessment for a specific sprint")
  ...
  @Operation(
      ...
      description = "Get historical velocity data for charting")
  public ResponseEntity<ApiResponse<VelocityDto.VelocityHistory>> getVelocityHistory(
      @CurrentUser UUID userId, @RequestParam(defaultValue = "10") int sprintCount) {
    return ResponseEntity.ok(
        ApiResponse.success(sensAIService.getVelocityHistory(userId, sprintCount)));
  }
  ...
  @Operation(
      summary = "Get life wheel metrics",
      description = "Get life wheel dimension scores and balance analysis")
  public ResponseEntity<ApiResponse<LifeWheelDto>> getLifeWheelMetrics(@CurrentUser UUID userId) {
  ...
  public ResponseEntity<ApiResponse<List<SprintCeremonyDto>>> getCeremoniesForSprint(
      @CurrentUser UUID userId, @PathVariable String sprintId) {
    return ResponseEntity.ok(
        ApiResponse.success(sensAIService.getCeremoniesForSprint(userId, sprintId)));
  }
  ...
  @Operation(
      summary = "Complete ceremony",
      description = "Complete a sprint ceremony with outcomes")
  ...
  @Operation(
      summary = "Process intake",
      description = "Process a universal intake request with AI categorization")
  public ResponseEntity<ApiResponse<IntakeDto>> processIntake(
      @CurrentUser UUID userId, @Valid @RequestBody IntakeDto.ProcessIntakeRequest request) {
