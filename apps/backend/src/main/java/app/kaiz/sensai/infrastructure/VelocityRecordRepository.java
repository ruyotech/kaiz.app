...
public interface VelocityRecordRepository extends JpaRepository<VelocityRecord, UUID> {
  ...
  @Query(
      "SELECT vr FROM VelocityRecord vr WHERE vr.user.id = :userId "
          + "ORDER BY vr.sprintStartDate DESC")
  List<VelocityRecord> findRecentVelocityRecords(@Param("userId") UUID userId, Pageable pageable);

  @Query("SELECT AVG(vr.completedPoints) FROM VelocityRecord vr " + "WHERE vr.user.id = :userId")
  Double getAverageCompletedPoints(@Param("userId") UUID userId);

  @Query("SELECT AVG(vr.completionRate) FROM VelocityRecord vr " + "WHERE vr.user.id = :userId")
  Double getAverageCompletionRate(@Param("userId") UUID userId);

  @Query("SELECT MAX(vr.completedPoints) FROM VelocityRecord vr " + "WHERE vr.user.id = :userId")
  Integer getBestSprintPoints(@Param("userId") UUID userId);

  @Query("SELECT SUM(vr.completedPoints) FROM VelocityRecord vr " + "WHERE vr.user.id = :userId")
  ...
  @Query(
      "SELECT COUNT(vr) FROM VelocityRecord vr WHERE vr.user.id = :userId "
