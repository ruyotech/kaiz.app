...
import app.kaiz.community.infrastructure.*;
...
public class CommunityService {
  ...
  public CommunityMemberResponse getMemberByUserId(UUID userId) {
    CommunityMember member =
        memberRepository
            .findByUserId(userId)
            .orElseThrow(
                () -> new IllegalArgumentException("Member not found for user: " + userId));
    return toMemberResponse(member);
  }
  ...
  public ArticleResponse createArticle(UUID authorId, CreateArticleRequest request) {
    ...
    if (article.getIsPublished()) {
      ...
      recordActivity(author, ActivityType.STORY_POSTED, "Published article: " + article.getTitle());
    }
  ...
  public MotivationGroupResponse createGroup(UUID creatorId, CreateGroupRequest request) {
    CommunityMember creator =
        memberRepository
            .findById(creatorId)
            .orElseThrow(() -> new IllegalArgumentException("Member not found: " + creatorId));
  ...
  public PartnerRequestResponse sendPartnerRequest(
    ...
    CommunityMember fromMember =
        memberRepository
            ...
            .orElseThrow(() -> new IllegalArgumentException("Member not found: " + fromMemberId));
    CommunityMember toMember =
        memberRepository
            .findById(request.toMemberId())
            .orElseThrow(
                () -> new IllegalArgumentException("Member not found: " + request.toMemberId()));
  ...
  public CommunityHomeResponse getCommunityHome(UUID userId) {
    ...
    List<CommunityActivityResponse> recentActivity =
        activityRepository.findTop10ByOrderByCreatedAtDesc().stream()
